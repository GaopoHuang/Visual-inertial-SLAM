...
def visualize_trajectory_2d(pose,path_name="Unknown",
			    show_ori=False,show=True,block=False,title='',get_posedata=False):
    '''
    ...
    n_pose = pose.shape[2]
    posedata1, = ax.plot(pose[0,3,:],pose[1,3,:],'r-',label=path_name)
    posedata2 = ax.scatter(pose[0,3,0],pose[1,3,0],marker='s',label="start")
    posedata3 = ax.scatter(pose[0,3,-1],pose[1,3,-1],marker='o',label="end")
    ...
    if get_posedata:
        return fig,ax, posedata1, posedata2, posedata3
    return fig, ax

def visualize_trajectory_with_landmarks_2d(pose, landmarks, path_name='unknown', 
					   show_ori=False, block=True,show=True,title='',get_posedata=False,
             xlim=None, ylim=None):
	returned_objects = visualize_trajectory_2d(pose, path_name, show_ori, 
				   show=False,title=title, get_posedata=get_posedata)
	fig, ax = returned_objects[0], returned_objects[1]
	landmark_vis = ax.scatter(landmarks[0,:], landmarks[1,:],s=1)
	if xlim is None:
		xlim = [-1500,1000]
		ylim = [-1500,1000]
	ax.set_xlim(xlim)
	...
	if show:
		plt.show(block=block)
		plt.pause(0.01)
	return returned_objects+(landmark_vis,)

def update_graph(poses, landmarks, t, graph_obs, show=False):
	fig, ax, ldmk_vis = graph_obs[0], graph_obs[1], graph_obs[-1] 
	if len(graph_obs)>3: 
		# pass 
		pd1, pd2, pd3 = graph_obs[2], graph_obs[3], graph_obs[4] 
		pd2.set_offsets(poses[:2,3,0].T)
		pd3.set_offsets(poses[:2,3,t].T)
		pd1.set_xdata(poses[0,3,:t].T)
		pd1.set_ydata(poses[1,3,:t].T)
	ldmk_vis.set_offsets(landmarks[:2,:].T)
	ax.set_title("t=%d"%t)
	if show:
		plt.pause(0.01)
...
def homocrdHat(hp):
  ...
  '''
  hatout = np.zeros(hp.shape+(6,))
  hatout[...,:3,:3] = np.tile(np.eye(3),(hp.shape[0],1,1)) 
  hatout[...,:3,3:] = -axangle2skew(hp[...,:3])
  return hatout 
